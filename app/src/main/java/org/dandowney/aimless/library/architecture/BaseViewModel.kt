package org.dandowney.aimless.library.architecture
...
abstract class BaseViewModel<UiEvent : ViewEvent, UiState : ViewState, UiEffect : ViewSideEffect> :
  ...
  protected suspend fun setState(reduce: suspend UiState.() -> UiState) {
    _state.value = _state.value.reduce()
  }

  protected suspend inline fun <reified T : UiState> setSealedState(crossinline reduce: (T) -> UiState) {
    setState {
      (this as? T)
        ?.let(reduce)
